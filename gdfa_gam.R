...
dat <- read.csv("~/ADNIMachineLearning/data/merged.csv")

# randomize order of rows 
dat <- dat[sample(nrow(dat)),]
rownames(dat) <- 1:nrow(dat) # re-adjust rownames 

dat$GDTOTAL <- as.numeric(dat$GDTOTAL > 4)
...
for (i in seq(1, 150, 10)) {
  ...
  b1 <- mgcv::gam(GDTOTAL ~ s(FA_CST_L, bs='ps', sp=0.3) + s(FA_CST_R, bs='ps', sp=0.3)
                  ...
                  family.mgcv = binomial(link = "logit"), # ordered categorical (ordinal) data
...
prediction_vet[prediction_vet > 1] <- 1
